...
contract NitroPool is ReentrancyGuard, Ownable, INFTHandler {
    ...
    using SafeERC20 for ISEldenToken;
    ...
    constructor(
        IEldenToken eldenToken_, ISEldenToken sEldenToken_, address owner_, INFTPool nftPool_,
        IERC20 rewardsToken1_, IERC20 rewardsToken2_, Settings memory settings_
    ) {
        require(address(eldenToken_) != address(0) && address(sEldenToken_) != address(0) && owner_ != address(0)
     ...
     * "to" can be set to this address only if this contract is allowed to transfer SElden
     */
    function onNFTHarvest(address operator, address to, uint256 tokenId, uint256 eldenAmount, uint256 sEldenAmount) external override isValidNFTPool(msg.sender) returns (bool) {
        ...
        if (to == address(this)) {
            eldenToken.safeTransfer(owner, eldenAmount);
            sEldenToken.safeTransfer(owner, sEldenAmount);
        }
