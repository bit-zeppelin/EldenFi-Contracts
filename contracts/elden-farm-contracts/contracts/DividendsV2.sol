...
/*
 * This contract is used to distribute dividends to users that allocated sElden here
 *
 ...
 * This distribution slot is the source of the dividends distribution to sElden allocators during the current cycle
 *
...
contract DividendsV2 is Ownable, ReentrancyGuard, ISEldenTokenUsage, IDividendsV2 {
  ...
  address public immutable sEldenToken; // sEldenToken contract

  mapping(address => uint256) public usersAllocation; // User's sElden allocation
  ...
  constructor(address sEldenToken_, uint256 startTime_) {
    require(sEldenToken_ != address(0), "zero address");
  ...
  modifier sEldenTokenOnly() {
    require(msg.sender == sEldenToken, "sEldenTokenOnly: caller should be SEldenToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of sElden to this dividends contract
   *
  ...
  /**
   * Deallocates "userAddress" user's "amount" of sElden allocation from this dividends contract
   *
   * Can only be called by sEldenToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IsEldenTokenUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no sElden is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userSEldenAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userSEldenAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userSEldenAllocation.mul(accDividendsPerShare).div(1e18);
