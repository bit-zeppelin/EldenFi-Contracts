...
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("Elden staking position NFT", "spNFT") {
  ...
  struct StakingPosition {
    ...
    uint256 boostPoints; // Allocated SElden from yieldboost contract (optional)
    uint256 totalMultiplier; // lockMultiplier + allocated SElden boostPoints multiplier
    uint256 pendingSEldenRewards; // Not harvested sElden rewards
    uint256 pendingEldenRewards; // Not harvested Elden rewards
  }
  ...
  ISEldenToken private _sEldenToken; // SEldenToken contract's address
  ...
  uint256 private _maxBoostMultiplier = 10000; // 100%, Max boost that can be earned from sElden yieldBooster

  uint256 private constant _TOTAL_REWARDS_SHARES = 10000; // 100%, high limit for sEldenRewardsShare
  uint256 public sEldenRewardsShare = 8000; // 80%, directly defines eldenShare with the remaining value to 100%
  ...
  function initialize(IEldenMaster master_, IERC20 eldenToken, ISEldenToken sEldenToken, IERC20 lpToken) external {
    require(msg.sender == factory && !initialized, "FORBIDDEN");
    _lpToken = lpToken;
    master = master_;
    _eldenToken = eldenToken;
    _sEldenToken = sEldenToken;
    initialized = true;

    // to convert ELDEN to SElden
   _eldenToken.approve(address(_sEldenToken), type(uint256).max);
  }
  ...
  event SetSEldenRewardsShare(uint256 sEldenRewardsShare);
  ...
  function getPoolInfo() external view override returns (
    address lpToken, address eldenToken, address sEldenToken, uint256 lastRewardTime, uint256 accRewardsPerShare,
    ...
    return (
    address(_lpToken), address(_eldenToken), address(_sEldenToken), lastRewardTime, _accRewardsPerShare,
  ...
  function pendingRewards(uint256 tokenId) external view returns (uint256) {
    ...
    return position.amountWithMultiplier.mul(accRewardsPerShare).div(1e18).sub(position.rewardDebt)
      .add(position.pendingSEldenRewards).add(position.pendingEldenRewards);
  }
  ...
  /**
   * @dev Set the share of SElden for the distributed rewards
   * The share of ELDEN will incidently be 100% - sEldenRewardsShare
   *
   * Must only be called by the owner
   */
  function setSEldenRewardsShare(uint256 sEldenRewardsShare_) external {
    _requireOnlyOwner();
    require(sEldenRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

    sEldenRewardsShare = sEldenRewardsShare_;
    emit SetSEldenRewardsShare(sEldenRewardsShare_);
  }
  ...
  function createPosition(uint256 amount, uint256 lockDuration) external override nonReentrant {
    ...
    _stakingPositions[currentTokenId] = StakingPosition({
      ...
      pendingEldenRewards: 0,
      pendingSEldenRewards: 0
    });
  ...
  function _harvestPosition(uint256 tokenId, address to) internal {
    ...
    // transfer rewards
    if (pending > 0 || position.pendingSEldenRewards > 0 || position.pendingEldenRewards > 0) {
      uint256 sEldenRewards = pending.mul(sEldenRewardsShare).div(_TOTAL_REWARDS_SHARES);
      uint256 eldenAmount = pending.add(position.pendingEldenRewards).sub(sEldenRewards);

      sEldenRewards = sEldenRewards.add(position.pendingSEldenRewards);

      // Stack rewards in a buffer if to is equal to address(0)
      if (address(0) == to) {
        position.pendingSEldenRewards = sEldenRewards;
        position.pendingEldenRewards = eldenAmount;
      }
      else {
        // convert and send SElden + ELDEN rewards
        position.pendingSEldenRewards = 0;
        position.pendingEldenRewards = 0;

        if(sEldenRewards > 0) sEldenRewards = _safeConvertTo(to, sEldenRewards);
        // send share of ELDEN rewards
        eldenAmount= _safeRewardsTransfer(to, eldenAmount);

        // forbidden to harvest if contract has not explicitly confirmed it handle it
        _checkOnNFTHarvest(to, tokenId, eldenAmount, sEldenRewards);
      }
    }
  ...
  */
  ...
  /**
   * @dev Safe convert ELDEN to SElden function, in case rounding error causes pool to not have enough tokens
   */
  function _safeConvertTo(address to, uint256 amount) internal returns (uint256) {
    ...
    if(amount > 0 ) _sEldenToken.convertTo(amount, to);
    return amount;
  }
   ...
   */
  function _checkOnNFTHarvest(address to, uint256 tokenId, uint256 eldenAmount, uint256 sEldenAmount) internal {
    address nftOwner = ERC721.ownerOf(tokenId);
    if (nftOwner.isContract()) {
      bytes memory returndata = nftOwner.functionCall(abi.encodeWithSelector(
          INFTHandler(nftOwner).onNFTHarvest.selector, msg.sender, to, tokenId, eldenAmount, sEldenAmount), "non implemented");
