...
contract YieldBooster is Ownable, ReentrancyGuard, ISEldenTokenUsage, IYieldBooster {
  ...
  // User's position SElden total allocation
  ...
  mapping(address => uint256) private _usersTotalAllocation; // User's SElden total allocation
  ...
  /**
   * @dev Returns total SElden allocated to this contract by "poolAddress"
   */
  ...
  /**
   * Allocates "userAddress" user's "amount" of SElden to this usage contract
  ...
  function deallocateAllFromPool(address userAddress, uint256 tokenId) external override nonReentrant {
    ...
    // update user's SElden allocations balance
    sEldenToken.deallocateFromUsage(userAddress, amount);
  }

  /**
   * Deallocates msg.sender's "amount" of SElden from SEldenToken, without adjusting this contract allocations balances
   *
  ...
  function forceDeallocate() external nonReentrant {
    ...
    uint256 amount = sEldenToken.usageAllocations(msg.sender, address(this));

    // update user's SElden allocations balance
    sEldenToken.deallocateFromUsage(msg.sender, amount);
  }
   ...
   * 1% of the pool's allocated SElden
   *
  ...
  function _deallocate(address userAddress, address poolAddress, uint256 tokenId, uint256 amount) internal {
    ...
    require(userPositionAllocation >= amount, "deallocate: not enough allocated sElden");
