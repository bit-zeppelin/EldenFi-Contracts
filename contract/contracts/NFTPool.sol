 ...
 * ksNFTs are yield-generating positions when the NFTPool contract has allocations from the EldenFi Master
 */
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("KamiStakedNFT", "ksNFT"), ERC721Enumerable {
  ...
  struct StakingPosition {
    ...
    uint256 boostPoints; // Allocated eldenStakeToken from yieldboost contract (optional)
    uint256 totalMultiplier; // lockMultiplier + allocated eldenStakeToken boostPoints multiplier
    uint256 pendingEldenStakeTokenRewards; // Not harvested eldenStakeToken rewards
    uint256 pendingEldenTokenRewards; // Not harvested Elden Token rewards
  }
  ...
  IEldenFiMaster public master; // Address of the master
  ...
  IERC20 private _eldenToken; // EldenToken contract's address
  IEldenStakeToken private _eldenStakeToken; // EldenStakeToken contract's address
  ...
  uint256 private _maxBoostMultiplier = 10000; // 100%, Max boost that can be earned from eldenStakeToken yieldBooster

  uint256 private constant _TOTAL_REWARDS_SHARES = 10000; // 100%, high limit for eldenStakeTokenRewardsShare
  uint256 public eldenStakeTokenRewardsShare = 8000; // 80%, directly defines eldenTokenShare with the remaining value to 100%
  ...
  function initialize(IEldenFiMaster master_, IERC20 eldenToken, IEldenStakeToken eldenStakeToken, IERC20 lpToken) external {
    require(msg.sender == factory && !initialized, "FORBIDDEN");
    _lpToken = lpToken;
    master = master_;
    _eldenToken = eldenToken;
    _eldenStakeToken = eldenStakeToken;
    initialized = true;

    // to convert Elden to eldenStakeToken
   _eldenToken.approve(address(_eldenStakeToken), type(uint256).max);
  }
  ...
  event SetEldenStakeTokenRewardsShare(uint256 eldenStakeTokenRewardsShare);
  ...
  function getPoolInfo() external view override returns (
    address lpToken, address eldenToken, address eldenStakeToken, uint256 lastRewardTime, uint256 accRewardsPerShare,
    ...
    return (
    address(_lpToken), address(_eldenToken), address(_eldenStakeToken), lastRewardTime, _accRewardsPerShare,
  ...
  function pendingRewards(uint256 tokenId) external view returns (uint256) {
    ...
    return position.amountWithMultiplier.mul(accRewardsPerShare).div(1e18).sub(position.rewardDebt)
      .add(position.pendingEldenStakeTokenRewards).add(position.pendingEldenTokenRewards);
  }
  ...
  /**
   * @dev Set the share of eldenStakeToken for the distributed rewards
   * The share of Elden will incidently be 100% - eldenStakeTokenRewardsShare
   *
   * Must only be called by the owner
   */
  function setEldenStakeTokenRewardsShare(uint256 eldenStakeTokenRewardsShare_) external {
    _requireOnlyOwner();
    require(eldenStakeTokenRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

    eldenStakeTokenRewardsShare = eldenStakeTokenRewardsShare_;
    emit SetEldenStakeTokenRewardsShare(eldenStakeTokenRewardsShare_);
  }
  ...
  function createPosition(uint256 amount, uint256 lockDuration) external override nonReentrant {
    ...
    _stakingPositions[currentTokenId] = StakingPosition({
      ...
      pendingEldenTokenRewards: 0,
      pendingEldenStakeTokenRewards: 0
    });
   ...
   * Users should still be able to deallocate eldenStakeToken from the YieldBooster contract
   */
  ...
  function _harvestPosition(uint256 tokenId, address to) internal {
    ...
    // transfer rewards
    if (pending > 0 || position.pendingEldenStakeTokenRewards > 0 || position.pendingEldenTokenRewards > 0) {
      uint256 eldenStakeTokenRewards = pending.mul(eldenStakeTokenRewardsShare).div(_TOTAL_REWARDS_SHARES);
      uint256 eldenTokenAmount = pending.add(position.pendingEldenTokenRewards).sub(eldenStakeTokenRewards);

      eldenStakeTokenRewards = eldenStakeTokenRewards.add(position.pendingEldenStakeTokenRewards);

      // Stack rewards in a buffer if to is equal to address(0)
      if (address(0) == to) {
        position.pendingEldenStakeTokenRewards = eldenStakeTokenRewards;
        position.pendingEldenTokenRewards = eldenTokenAmount;
      }
      else {
        // convert and send eldenStakeToken + Elden rewards
        position.pendingEldenStakeTokenRewards = 0;
        position.pendingEldenTokenRewards = 0;

        if(eldenStakeTokenRewards > 0) eldenStakeTokenRewards = _safeConvertTo(to, eldenStakeTokenRewards);
        // send share of Elden rewards
        eldenTokenAmount= _safeRewardsTransfer(to, eldenTokenAmount);

        // forbidden to harvest if contract has not explicitly confirmed it handle it
        _checkOnNFTHarvest(to, tokenId, eldenTokenAmount, eldenStakeTokenRewards);
      }
    }
  ...
  */
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256) {
    uint256 balance = _eldenToken.balanceOf(address(this));
    // cap to available balance
    if (amount > balance) {
      amount = balance;
    }
    _eldenToken.safeTransfer(to, amount);
    return amount;
  }

  /**
   * @dev Safe convert Elden to eldenStakeToken function, in case rounding error causes pool to not have enough tokens
   */
  function _safeConvertTo(address to, uint256 amount) internal returns (uint256) {
    ...
    if(amount > 0 ) _eldenStakeToken.convertTo(amount, to);
    return amount;
  }
   ...
   */
  function _checkOnNFTHarvest(address to, uint256 tokenId, uint256 eldenTokenAmount, uint256 eldenStakeTokenAmount) internal {
    address nftOwner = ERC721.ownerOf(tokenId);
    if (nftOwner.isContract()) {
      bytes memory returndata = nftOwner.functionCall(abi.encodeWithSelector(
          INFTHandler(nftOwner).onNFTHarvest.selector, msg.sender, to, tokenId, eldenTokenAmount, eldenStakeTokenAmount), "non implemented");
