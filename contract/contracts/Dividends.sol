...
/*
 * This contract is used to distribute dividends to users that allocated xElden here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xElden allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IxEldenTokenUsage, IDividends {
  ...
  address public immutable xEldenToken; // xEldenToken contract

  mapping(address => uint256) public usersAllocation; // User's xElden allocation
  ...
  constructor(address xEldenToken_, uint256 startTime_) {
    require(xEldenToken_ != address(0), "zero address");
  ...
  modifier xEldenTokenOnly() {
    require(msg.sender == xEldenToken, "xEldenTokenOnly: caller should be xEldenToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of xElden to this dividends contract
   *
  ...
  /**
   * Deallocates "userAddress" user's "amount" of xElden allocation from this dividends contract
   *
   * Can only be called by xEldenToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IxEldenTokenUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no xElden is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userxEldenAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userxEldenAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userxEldenAllocation.mul(accDividendsPerShare).div(1e18);
