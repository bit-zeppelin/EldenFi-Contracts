...
/*
 * This contract centralizes EldenFi's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the EldenToken contract.
 */
contract EldenFiMaster is Ownable, IEldenFiMaster {
  ...
  IEldenToken private immutable _eldenToken; // Address of the Elden token contract
  ...
  constructor(
    IEldenToken eldenToken_,
    uint256 startTime_
  ) {
    require(address(eldenToken_) != address(0), "EldenFiMaster: EldenToken cannot be set to zero address");
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= eldenToken_.lastEmissionTime(), "EldenFiMaster: invalid startTime");
    _eldenToken = eldenToken_;
    startTime = startTime_; // Must be set with the same time as EldenToken emission start
  }
  ...
  /*
   * @dev Returns EldenToken address
   */
  function eldenToken() external view override returns (address) {
    return address(_eldenToken);
  }

  /*
   * @dev Returns EldenToken's emission rate (allocated to this contract)
   */
  function emissionRate() public view returns (uint256) {
    return _eldenToken.masterEmissionRate();
  }

  /**
   * @dev Returns current owner's address
   */
  function owner() public view virtual override(IEldenFiMaster, Ownable) returns (address) {
  ...
  function setStartTime(uint256 startTime_) external onlyOwner {
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= _eldenToken.lastEmissionTime(), "EldenFiMaster: invalid startTime");
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256 effectiveAmount) {
    uint256 eldenBalance = _eldenToken.balanceOf(address(this));

    if (amount > eldenBalance) {
      amount = eldenBalance;
    }
  ...
  function _updatePool(address poolAddress) internal {
    ...
    if (allocPoint > 0 && INFTPool(poolAddress).hasDeposits()) {
      // calculate how much Elden rewards are expected to be received for this pool
      ...
      (rewards) = _eldenToken.claimMasterRewards(rewards);
