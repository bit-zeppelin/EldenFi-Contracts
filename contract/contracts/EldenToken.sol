...
/*
 * Elden is EldenFi's native ERC20 token.
 * It has an hard cap and manages its own emissions and allocations.
 */
contract EldenToken is Ownable, ERC20("Elden Token", "ELDEN"), IEldenToken {
  using SafeMath for uint256;

  uint256 public constant MAX_EMISSION_RATE = 0.01 ether;
  uint256 public constant MAX_SUPPLY_LIMIT = 2000000 ether;
   ...
   */
  modifier onlyMaster() {
    require(msg.sender == masterAddress, "EldenToken: caller is not the master");
    _;
  }
  ...
  /**
   * @dev Burns "amount" of Elden by sending it to BURN_ADDRESS
   */
  ...
  /**
   * @dev Updates Elden emission rate per second
   *
   * Must only be called by the owner
   */
