...
contract RaitoPool is ReentrancyGuard, Ownable, INFTHandler {
    ...
    using SafeERC20 for IEldenStakeToken;
    ...
    IEldenStakeToken public eldenStakeToken; // eldenStakeToken contract
    ...
    constructor(
        IEldenToken _eldenToken, IEldenStakeToken eldenStakeToken_, address owner_, INFTPool nftPool_,
        IERC20 rewardsToken1_, IERC20 rewardsToken2_, Settings memory settings_
    ) {
        require(address(_eldenToken) != address(0) && address(eldenStakeToken_) != address(0) && owner_ != address(0)
     ...
     * "to" can be set to this address only if this contract is allowed to transfer eldenStakeToken
     */
    function onNFTHarvest(address operator, address to, uint256 tokenId, uint256 eldenTokenAmount, uint256 eldenStakeTokenAmount) external override isValidNFTPool(msg.sender) returns (bool) {
        address owner = tokenIdOwner[tokenId];
        require(operator == owner, "not allowed");

        // if not whitelisted, the RaitoPool can't transfer any eldenStakeToken rewards
        require(to != address(this) || eldenStakeToken.isTransferWhitelisted(address(this)), "cant handle rewards");

        // redirect rewards to position's previous owner
        if (to == address(this)) {
            eldenToken.safeTransfer(owner, eldenTokenAmount);
            eldenStakeToken.safeTransfer(owner, eldenStakeTokenAmount);
        }
